...
export default function AddVisit({ workerName, dateVisit, timeStart, timeEnd, close }) {
    ...
    const { worker, date, start, end, services, customer } = newVisit;
            ...
            <Form onChange={handleChange} onSubmit={handleSubmit}>
                ...
                <Label className="customer">
                    ...
                    <select defaultValue={visitCustomer} onChange={(e) => setVisitCustomer(e.target.value)} name="customer" required>
                            <option></option>
                        {customers.map(({firstName, lastName, id}) => {
                ...
                {/* <Label className="customer">
                    <span>Klient: </span>
                    <input type="text" onChange={(e) => findCustomer(e, setFilteredCustomers, customers)} name="customer" defaultValue={customer} autoComplete="off" required />
                    {(filteredCustomers.length !== 0 && filteredCustomers.length < customers.length) && (
                    <div className="filtered">
                        <ul>
                                {filteredCustomers.map(({ firstName, lastName, id }) => (
                                    <a key={id} onClick={getValue} value={lastName}>{firstName} {lastName}</a>
                            ))}
                        </ul>
                    </div>
                    )}
                ...
                <Label className="end">
                    ...
                    <select defaultValue={end} name="end">
                        {hours.map(({ end }) => (<option key={end} value={end}>{end}</option>))}
                    </select>
                </Label>
